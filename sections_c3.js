...
window.showMallaDetails = function(comp, asigIdx, btn) {
    ...
    document.getElementById('ra_perfil').innerHTML = data.perfil ? `<i class="fas fa-arrow-circle-right text-toOrange mr-2"></i><strong>${data.perfil.split('.')[0]}.</strong> ${data.perfil.substring(data.perfil.indexOf('.') + 1).trim()}` : '-';
};
...
<div id="malla_traz" class="tab-content">
    ...
    <div style="background:#0A2540;border-radius:14px;padding:20px;color:white;position:relative;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,.08);">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;position:relative;margin-bottom:15px;">
        ...
        <div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:16px;position:relative;">
            <h5 style="color:#FF6600;font-weight:700;font-size:.75rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;"><i class="fas fa-star mr-2"></i>Contribución al Perfil Específico del Egresado (Puntos 9 al 16)</h5>
            <p id="ra_perfil" style="color:#e2e8f0;font-size:.85rem;line-height:1.5;margin:0;">-</p>
        </div>
    </div>
</div>
`;
